...
def extraer_antecedentes(df: pd.DataFrame) -> pd.DataFrame:
    out = _select(df, {
        ...
        "FA":          "FA_num",
        # BUG-D FIX (v5.0): config.py renombra 'Etiología' → 'Etiologia' (sin tilde)
        "Etiologia":   "Etiologia",
    })
...
def extraer_tiempos_llegada(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # MEJORA-1 (v5.0): Convertir fracciones decimales de Excel a HH:MM
    def fraccion_a_hhmm(val):
        try:
    ...
    cols_hora = ["Horapuerta", "HoraTriaje", "LlamadaNeuro", "ValoradoNeuro",
                 "HoraTC", "ultimaimagenTC"]
    for col in cols_hora:
        if col in out.columns:
            out[col] = out[col].apply(fraccion_a_hhmm)
...
def extraer_tiempos_intervencion(df: pd.DataFrame) -> pd.DataFrame:
    out = _select(df, {
        "NHC":             "NHC",
        "FECHA":           "FECHA",
        "HoraPunción":     "HoraPuncion",   # col limpia con tilde (210 datos)
...
def extraer_intervalos(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Limites clinicos razonables por intervalo (en minutos)
    LIMITES_CLINICOS = {
        "T_SintomasPuerta":        (1,  1440),
        "T_PuertaValoracion":      (1,   300),
        "T_LlamadaValoracion":     (1,   300),
        "T_ValoraciónTC":          (1,   300),
        "T_PuertaTC":              (1,   300),
        "T_TC":                    (1,   120),
        "T_PuertaSala":            (1,   400),
        "T_TCSala":                (1,   300),
        "T_PuertaPuncion":         (1,   400),
        "T_SalaPuncion":           (1,   300),
        "T_PuertaRecanalizacion":  (1,   600),
        "T_Puncion1Pase":          (1,   300),
        "T_PuncionRecanalizacion": (1,   300),
        "T_SintomasRecanalizacion":(1,  1440),
    }
    ...
    for col in cols_intervalo:
        ...
        # 1. Convertir formato HH:MM o HH:MM:SS a minutos
        ...
        if mask_hhmmss.any():
            ...
            out.loc[mask_hhmmss, col] = minutos.values

        # 2. Convertir a numerico
        out[col] = pd.to_numeric(out[col], errors="coerce")

        # 3. Detectar y convertir fracciones decimales de Excel (0.0 - 1.0 = fraccion de dia)

        mask_fraccion = out[col].notna() & (out[col] > 0) & (out[col] < 1)
        if mask_fraccion.any():
            n_frac = mask_fraccion.sum()
            out.loc[mask_fraccion, col] = (out.loc[mask_fraccion, col] * 24 * 60).round()
            log.info(f"    {col}: {n_frac} fracciones Excel convertidas a minutos")

        # 4. Aplicar limites clinicos — valores fuera de rango a NaN
        if col in LIMITES_CLINICOS:
            vmin, vmax = LIMITES_CLINICOS[col]
            mask_invalido = out[col].notna() & ~out[col].between(vmin, vmax)
            if mask_invalido.any():
                log.warning(
                    f"    {col}: {mask_invalido.sum()} valores fuera de rango "
                    f"[{vmin},{vmax}] min → NaN "
                    f"(ejemplos: {sorted(out.loc[mask_invalido, col].dropna().tolist())[:5]})"
                )
                out.loc[mask_invalido, col] = pd.NA
...
def run():
    log.info("=" * 60)
    log.info("FASE 4 -- CARGA EN SQL SERVER  (v5.0)")
    log.info("=" * 60)
